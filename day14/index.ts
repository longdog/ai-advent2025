...
2. **Testing Framework**
   ...
   - DONT USE functions from jest or OTHER non "bun:test" test runners!
   - Use mock from "bun:test" for mocking. Import mock from "bun:test". Example: const random = mock(() => Math.random());
...
{
  "tests": {
    "src/utils/math.test.ts": "import { test, expect, mock } from 'bun:test';\\nimport { add } from './math';\\n\\ntest('add works', () => { expect(add(2, 3)).toBe(5); });"
  }
}
`;
...
async function generateTests(model: any, context: string) {
  const messages = [new SystemMessage(systemPrompt), new HumanMessage(context)];
  const req = await model.invoke(messages);
...
async function main() {
  ...
  const modelName = process.argv[3] || "groq";
  const model = modelName === "groq" ? groq : giga;
  ...
  console.log(`🤖 Sending project to ${modelName} model...`);
  const raw = await generateTests(model, projectContext);
